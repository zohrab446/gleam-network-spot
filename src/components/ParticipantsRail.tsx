    ...
    >
      <div className="card-surface flex min-h-0 flex-1 flex-col p-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base">Katılımcılar</h2>
        ...
        {gap !== null && (
          <p className="mt-3 rounded-xl bg-primary-soft px-3 py-2 text-xs font-bold text-primary">
            {gap === 0
              ? `${ahead?.username} ile başabaşsın! 🔥`
              : `${ahead?.username}'i geçmek için ${gap} XP kaldı 🚀`}
          </p>
        )}
        ...
        <ol className="mt-3 min-h-0 flex-1 space-y-1.5 overflow-y-auto pr-1">
          ...
          {!isLoading && rows.length === 0 && (
            <li className="text-xs text-muted-foreground">Henüz katılımcı yok. İlk sen ol!</li>
          )}
        </ol>
        ...
        >
          <Trophy className="h-4 w-4" /> Tüm tabloyu gör
        </Link>
      </div>
    </aside>
  );
